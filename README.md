# One-Source-Awareness-Model
The One-Source Awareness Model (OSAM): a self-consistent ontological framework for consciousness differentiation and worldview construction.
[一元同源觉知推理模型.pdf](https://github.com/user-attachments/files/30594990/default.pdf)
[一元同源觉知推演模型原稿.pdf](https://github.com/user-attachments/files/30595097/default.pdf)
## 重要说明 Notice
1. **权威原版**
本理论唯一基准文本为中文PDF文档。所有歧义、争议，均以中文原版内容为准。

2. **翻译相关**
仓库暂不提供官方英文全文译本。
依据 CC-BY 4.0 协议，读者可以自行翻译用于个人研究。
⚠️ 机器翻译极易扭曲核心专有概念；所有非官方译文未经作者校核，仅作参考。

> The canonical version of the OSAM is the Chinese PDF.
> No official full English translation is provided. Readers may translate independently under CC-BY 4.0.
> Machine translation may distort core concepts. Unofficial translations are not reviewed by the author.

3. **使用规范**
- 允许引用、学习、二次推演；二次传播、衍生创作**必须标注原项目与作者来源**。
- 禁止篡改理论内容后冒充原版体系；禁止剥离出处，包装为付费课程、玄学体系进行牟利。

## License
Content of this repository is licensed under **Creative Commons Attribution 4.0 International (CC-BY 4.0)**.
## 定位

- **性质**：公共思想资源 · 幻想创作者工具箱 · 形而上思辨框架
- **状态**：V1.0 · 2026年8月 · 首发于 GitHub / Zenodo
- **作者**：异帧玄造
- **DOI**：（待 Zenodo 生成后填入）

---

## 体系结构

本仓库包含以下文档：

| 编号 | 篇目 | 定位 |
| :--- | :--- | :--- |
| 1 | 引子 | 无门槛入口 |
| 2 | 六条公理 | 核心起点 |
| 3 | 符号与方程 | 骨架展示 |
| 4 | 离散化演示 | 骨架动起来 |
| 5 | 世界观模板 | 可复用工具 |
| 6 | 使用手册 | 无门槛出口 |
| 7 | 文字推演选录 | 深层思辨片段 |
| 8 | 原始文档附注版 | 完整工作底稿 |
| 9 | 延伸对照 | 与外部体系的边界对话 |

---

## 原创声明 & 授权

本文全部文字内容为原创，享有完整著作权。

本套觉知公理推演框架作为公共思想资源，欢迎所有幻想创作者、思辨爱好者免费借鉴、使用、拓展。

**禁止**：
- 未经许可完整转载、摘抄全文用于付费课程、付费文档、商业出版物；
- 截取文本断章取义，将本模型包装为灵修、玄学、伪科学内容对外传播；
- 篡改原文歪曲体系定位后二次发布。

**建议**：引用、借鉴时标注来源，便于同好溯源交流。

---

## 版本记录

| 版本 | 日期 | 说明 |
| :--- | :--- | :--- |
| V1.0 | 2026.08 | 完整九篇首发 |

---

## 联系方式

（暂不公开）

---

## 授权协议

本仓库内容采用 [CC BY-NC-ND 4.0](https://creativecommons.org/licenses/by-nc-nd/4.0/) 协议。
